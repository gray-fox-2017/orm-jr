...
describe('CREATE STUDENT', function() {
  it('function run well', function() {
    Student.create(dbModel.connection, new Student({first_name:'Antoni', last_name:'Angga', id_cohort:1}));
  })
})
describe('UPDATE STUDENT', function() {
  it('function run well', function() {
    Student.update(dbModel.connection, new Student({first_name:'Antoni', last_name:'Angga', id_cohort:1, id:1}));
  })
})
...
describe('FINDORCREATE STUDENT', function() {
  it('function run well', function() {
    Student.findOrCreate(dbModel.connection, new Student({first_name: 'Antoni', last_name: 'Angga', id_cohort:2}));
  })
})
...
describe('CREATE COHORT', function() {
  it('function run well', function() {
    Cohort.create(dbModel.connection, new Cohort({name:'Line group'}));
  })
})
describe('UPDATE COHORT', function() {
  it('function run well', function() {
    Cohort.update(dbModel.connection, new Cohort({name:'Line group', id:1}));
  })
})
...
describe('WHERE COHORT', function() {
  it('function run well', function(done) {
    Cohort.where(dbModel.connection, {name: 'Line group'}, done);
  })
})
